...
final class MultitouchFrameProvider: TrackpadFrameProvider, @unchecked Sendable {
    ...
    private var rawHandle: ASMultitouchHandle?
    ...
    func start(onFrame: @escaping @Sendable (RawTrackpadFrame) -> Void) throws {
        ...
        guard ASMultitouchFrameworkIsAvailable() else {
        ...
        guard let handle = ASMultitouchCreate(multitouchFrameProviderCallback, context) else {
        ...
        let status = ASMultitouchStart(handle)
        guard status == 0 else {
            ASMultitouchDestroy(handle)
            Unmanaged<MultitouchCallbackContext>.fromOpaque(context).release()
            frameHandler = nil
            throw TrackpadFrameProviderError.unavailable(Self.bridgeError())
        }
    ...
    func stop() {
        if let rawHandle {
            ASMultitouchStop(rawHandle)
            ASMultitouchDestroy(rawHandle)
        }
    ...
    fileprivate func receive(
        touches: UnsafePointer<ASRawTouch>?,
        touchCount: Int32,
        timestamp: Double,
        frame: Int32
    ) {
    ...
    private static func bridgeError() -> String {
        guard let pointer = ASMultitouchLastError() else {
...
private func multitouchFrameProviderCallback(
    _ touches: UnsafePointer<ASRawTouch>?,
    _ touchCount: Int32,
    _ timestamp: Double,
    _ frame: Int32,
