#include "AirShortcutMultitouchBridge.h"
...
static ASMultitouchContext *activeContext = NULL;
...
static void bridgeFrameCallback(
    ...
    ASMultitouchContext *context = activeContext;
    ...
    ASRawTouch normalized[maximumTouchCount];
    for (int32_t index = 0; index < count; index += 1) {
        MTTouch touch = touches[index];
        normalized[index] = (ASRawTouch) {
...
ASMultitouchHandle ASMultitouchCreate(
    ...
    ASMultitouchContext *context = calloc(1, sizeof(ASMultitouchContext));
...
int32_t ASMultitouchStart(ASMultitouchHandle handle) {
    ASMultitouchContext *context = (ASMultitouchContext *)handle;
    if (context == NULL) {
